...
export function CapitalTestQuiz({ countryNames, elapsedMs }: CapitalTestQuizProps) {
  ...
  const handleSubmit = () => {
    if (!currentQuestionId || !countryName || !capitalName || showResult) return;
      ...
      {showResult && lastResult ? (
        <div className={styles.result}>
          ...
          <Button variant="primary" fullWidth onClick={handleNext}>
            {answers.length + 1 >= questionQueue.length ? 'Zobacz wyniki' : 'Następne pytanie'}
          </Button>
        </div>
      ) : (
        <form
